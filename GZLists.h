...
void GZSList<Mydata>::EmptyList() {
  ...
  while (NULL != p) {
    p_next = p->next;
    delete p->element;  // Удаляем данные
    delete p;           // Удаляем обёртку
    p = p_next;
  }
...
class GZTListable {
  template <class Mydata>
  friend class GZTList;

 public:
  Mydata *element;
  __int64 timestamp;
  GZTListable(Mydata *_element, __int64 _timestamp)
      : element(_element), timestamp(_timestamp), next(NULL), prev(NULL){};
  ~GZTListable() {
    delete element;
  }  // в отличие от простого списка - элемент удаляется
...
class GZTList {
 public:
  ...
  // GZTListable<Mydata> *Insert(Mydata *_element, __int64 _timestamp, int
  // index); // Вставить по конкретному номеру GZTListable<Mydata>
  // *Closest(__int64 _timestamp, __int64 range=10000); // поиск в диапазоне
  // 1/100 секунды
};
...
//===========================================================
template <class Mydata>
GZTListable<Mydata> *GZTList<Mydata>::Add(Mydata *_element,
                                          __int64 _timestamp) {
  // С этого начинаем, чтобы потом не забыть
  num_elements++;
  ...
  GZTListable<Mydata> *p = first,
                      *p_new = new GZTListable<Mydata>(_element, _timestamp);

  while (NULL != p)  // p - валидный элемент списка, кандидат на следующий по
                     // возрастанию
  {
    if (p->timestamp
        > _timestamp)  // наткнулись на элемент, чей timestamp больше нашего
    {
    ...
    p = p->next;  // берём либо переменную first в самом списке, либо переменную
                  // next в элементе списка
  }
