...
class WorkerThread {
 public:
  static void Start(
      HWND hdwnd = 0);  // Создаёт поток и ждёт блоков данных от InputThread
  ...
  static int PullData(
      short *_buf);  // Отдаёт данные, если готовы (готовы returns 0)

  static volatile int
      indicator_value;  // принимает значение от 0 до 5 в зависимости от уровня
 ...
 protected:
  ...
  static volatile char flag_input_buffer_ready;
  static CRITICAL_SECTION cs_input_buf;
