...
TCHAR *MMWAVLoader::LoadWavFile(TCHAR *_filename, HWND hdwnd) {
  ...
  // !!! Здесь нужно приостановить FillBuffer - а ещё правильнее вообще
  // выключить его на время смены источника
...
void MMWAVLoader::snd_left_right() {
  ...
  if (position >= length)
    position -= length;  // не так, как в FillBuffer, после заворота файл может
                         // читаться не с нулевой позиции
}
//===================================================================================
...
void MMWAVLoader::FillBuffer(short *input_buf) {
      ...
      > length)  // Нужно копировать двумя частями и заворачивать
  {
    ...
    CopyMemory(
        input_buf, &samples[position], (length - position) * sizeof(short));
    // Копируем второй кусок, уже из начала
    CopyMemory(input_buf,
               &samples[0],
               (MM_SOUND_BUFFER_LEN - (length - position)) * sizeof(short));

    // Возможна пробуксовка
    if (flag_pause) {
      // Хемминг отменён, так как не выполнил возложенные на него задачи
      // [13-DEC]
      ...
      position = MM_SOUND_BUFFER_LEN - (length - position);
