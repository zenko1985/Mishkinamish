...
bool flag_sphinx_legacy = false;  // Используем ли старый кривоватый алгоритм
                                  // Сфинкса для вычисления DCT?
...
extern MModel model;
LONG volatile sounds_found[6] = {
    0};  // самая главная переменная во всей программе
...
static Ipp32f buf_out_balovstvo[DFT_SIZE + 2] = {
    0};  // Чтобы сюда inline преобразовать (формат CCS)
static Ipp32f buf_out_mfcc[DFT_SIZE + 2] = {
    0};  // один буфер - для баловства со сдвигом спектра, другой - для mfcc

static float buf_cas_MEL[40] = {
    0};  // для представления спектра мощности в MEL-координатах
...
static void fallback_dct(float *data, int n) {
  if (n > 40) n = 40;  // Защита от переполнения tmp[40]
  float tmp[40];
...
float cas_pre_emphasis(float *frame, float prev_emp) {
  ...
  // Тут пришлось перебирать в обратном порядке, чтобы оперировать неизменёнными
  // данными (ибо работаем в одном буфере)
...
void CopyShmopy::Process(short *dest, short *src) {
  ...
  // 1.1. Копируем во вторую половину буфера (заодно конвертируем из short во
  // float): 1.1.1. [пока под вопросом] Если ввод НЕ с микрофона, нужно поделить
  ...
  while (input_offset <= MM_SOUND_BUFFER_LEN) {
    ...
    if (MM_SOUND_BUFFER_LEN == input_offset) {
      prev = super_prev;  // берём реальное значение из предыдущего фрейма, а не
                          // из себя зацикленного [14-DEC]
      frame_border = true;
    } else
      frame_border = false;

    // 1.3. Отфурьируем
    // Вот здесь была ошибка. У нас Фурье обрабатывает DFT_SIZE отсчётов, а мы
    // давали ему указатель на буфер на 102 отсчета меньше. Пока со спектром не
    // баловались - это было не заметно, а потом - вылезло.
    // magic_data=DoMagic(&InputDoubleBuffer[input_offset]);

    memcpy(buf_in,
           &InputDoubleBuffer[input_offset],
           MM_SOUND_BUFFER_LEN * sizeof(float));
    ...
    // В случае зацикливания звука (flag_pause) и начала блока (frame_border)
    // вычисленные MFCC-коэффициенты запоминаем в master_mfcc
    magic_data = DoMagic(buf_in, frame_border && flag_pause);

    // Вариант 2
    // просто копируем то, что преэмфасили и отхемминговали, но усиливаем в 512
    // раз!!!
    ...
    // 1.4. Наложим слой в выходной буфер (если всё испеклось, то там заполним и
    // dest)
  ...
  // 1.6. Сдвигаем вторую половину буфера в первую часть, смещение тоже
  // корректируем
  memcpy(&InputDoubleBuffer[0],
         &InputDoubleBuffer[MM_SOUND_BUFFER_LEN],
         MM_SOUND_BUFFER_LEN * sizeof(float));
...
void CopyShmopy::IntegrateMagic160(float *magic_data, short *dest) {
  int i;
  const int buf_size = MM_SOUND_BUFFER_LEN * 2;  // Размер OutputDoubleBuffer

  // Восходящий участок 90-160
  for (i = 90; i < 160; i++) {
    int idx = output_offset + i;
    if (idx >= 0 && idx < buf_size)
      OutputDoubleBuffer[idx] += magic_data[i] * (i - 90) / 70.0f;
  }
  // Нисходящий участок 250-320
  for (i = 250; i < 320; i++) {
    int idx = output_offset + i;
    if (idx >= 0 && idx < buf_size)
      OutputDoubleBuffer[idx] += magic_data[i] * (320 - i) / 70.0f;
  }
  // Ровный участок 160-250
  for (i = 160; i < 250; i++) {
    int idx = output_offset + i;
    if (idx >= 0 && idx < buf_size)
      OutputDoubleBuffer[idx] = magic_data[i];
  }

  // В следующий раз надо бы писать сюда..
  output_offset += 160;
  // Однако, выдаём результат, если смещение >=410, сдвигаем и уменьшаем
  // смещение
  if (output_offset >= MM_SOUND_BUFFER_LEN) {
    ...
    // корректируем
    memcpy(&OutputDoubleBuffer[0],
           &OutputDoubleBuffer[MM_SOUND_BUFFER_LEN],
           MM_SOUND_BUFFER_LEN * sizeof(float));
    output_offset -= MM_SOUND_BUFFER_LEN;

    // Важно! Обнуляем вторую половину буфера!
    ZeroMemory(&OutputDoubleBuffer[MM_SOUND_BUFFER_LEN],
               MM_SOUND_BUFFER_LEN * sizeof(float));
  }
}
...
float *CopyShmopy::DoMagic(float *input_data, bool remember_master_mfcc) {
  ...
  /*	Это всё вынесено в функцию CS_mel_cep, т.к. этот код вызывается ещё и в
     конструкторе модели
          // 4.1 Логарифм
          for (i = 0; i < MEL_NUM_FILTERS; i++)
          {
          buf_cas_MEL[i] = (float)(log(buf_cas_MEL[i] + 1e-4)); // #define
     LOG_FLOOR 1e-4 - так было в безвременно ушедшем Сфинксе
      }
          // 4.1.5 Для legacy:
  ...
  */
  ...
  if (remember_master_mfcc)
    master_mfcc = *((mfcc_t *)buf_cas_cep);  // Те же 13 float, но тип другой

  // 4.6. Либо тренируемся, либо определяем, наш ли это звук?
  if (flag_training_mode) {
    if (training_frame_counter >= 40) {
      if ((WorkerThread::training_silence_indicator + 1
           < WorkerThread::indicator_value))  // Пришла пора записывать, если
                                              // уровень достаточен (тишина +2
                                              // деления)
      {
        // Нужно создать (!) mfcc, скопировать в него значения, а потом уж
        // добавить в модель
        ...
        *new_mfcc = *((mfcc_t *)buf_cas_cep);
        ...
        // Для звуков "к" и "Ч" тренировка короче (сейчас одинакова для обоих,
        // но может быть разная)
  ...
  } else if (WorkerThread::flag_sound_detection_enabled)  // Проверка на попадание (только при достаточном уровне)
  {
    ...
    i = model.WhichSound(
        (mfcc_t *)buf_cas_cep,
        true);  // Если попало в 2 области - берём первую попавшуюся
    if (i >= 0) {
      InterlockedIncrement(
          &sounds_found[i]);  // Ибо другой поток сбрасывает в 0
                              // (Indicators::Draw - какая насмешка! Плебей
                              // принимает решение!)
      // [06.04.2019]
  ...
  mfcc_buffer1[mfcc_pointer] =
      *((mfcc_t *)buf_cas_cep);  // Те же 13 float, но тип другой
  mfcc_pointer++;
  if (mfcc_pointer >= MM_MFCC_BUFFER_LEN) {
    ...
    if (flag_mfcc_buffer2_empty) {
      ...
      for (i = 0; i < MM_MFCC_BUFFER_LEN; i++)  // Все MFCC в подборке
      {
        ...
        {
          if (mfcc_buffer2[i].coeff[k] < CopyShmopy_minmax[k][0])
            CopyShmopy_minmax[k][0] = mfcc_buffer2[i].coeff[k];
          if (mfcc_buffer2[i].coeff[k] > CopyShmopy_minmax[k][1])
            CopyShmopy_minmax[k][1] = mfcc_buffer2[i].coeff[k];
        }
      }
      // CopyShmopy_minmax
#ifdef MM_SUPERUSER
      InvalidateRect(
          exported_ogl_hwnd, NULL, FALSE);  // Рисование в отладочном окне
#endif
    } else {
  ...
  //============== Оставим немного баловства со сдвигом спектра и голосом Чипа и
  //Дейла ==========
  /*
...
ippstatus=ippsDFTInv_CCSToR_32f(buf_out_balovstvo, buf_out_balovstvo, pDFTSpec,
pDFTWorkBuf);
  */
  // Отключили баловство - обратное преобразование Фурье - и сэкономили 400Kбайт
  // в размере EXE Не говоря уже об экономии процессорных мощностей
  for (i = 0; i < MM_SOUND_BUFFER_LEN; i++)
...
void CopyShmopy::CS_mel_cep(float *mfspec, float *mfcep) {
  ...
  for (i = 0; i < MEL_NUM_FILTERS; i++) {
    mfspec[i] =
        (float)(log(mfspec[i] + 1e-4));  // #define LOG_FLOOR 1e-4 - так было в
                                         // безвременно ушедшем Сфинксе
  }
  // 4.1.5 Для legacy:
