...
/* int Indicators::xpos_K, Indicators::ypos_K, Indicators::xpos_I,
Indicators::ypos_I; int Indicators::xpos_E, Indicators::ypos_E,
Indicators::xpos_U, Indicators::ypos_U; int Indicators::xpos_O,
Indicators::ypos_O, Indicators::xpos_CH, Indicators::ypos_CH;
*/
...
void Indicators::Init(HWND _hdwnd) {
  hdwnd = _hdwnd;
  // RECT rect={158,26,145,48}; // xpos,ypos,xsize,ysize, а никакой не rect.
  // Нужно для DLU
  RECT rect = {
      158,
      26,
      ...
      8
          * (num_steps
             + 1)};  // xpos,ypos,xsize,ysize, а никакой не rect. Нужно для DLU
  ...
  hbm2 = CreateCompatibleBitmap(
      hdc,
      rect.right,
      rect.bottom / (num_steps + 1));  // Для полоски прогресса записи
  SelectObject(memdc, hbm);
  SelectObject(memdc2, hbm2);
  ReleaseDC(_hdwnd, hdc);

  // Рисуем на битмапе зелёных чело.. индикаторов на черном фоне + красная
  // перегрузка
  ...
  for (int i = 0; i < num_steps - 1; i++) {
    ...
    rect.top += vert_increase;
    rect.bottom += vert_increase;
  ...
  for (i = 1; i < num_steps; i++) {
    ...
    for (j = 0; j < i; j++) {
      ...
      // Глаз 1
      MoveToEx(memdc,j*hor_increase+hor_increase/3,
      i*vert_increase+vert_increase/3, NULL);
      LineTo(memdc,j*hor_increase+hor_increase/3+1,
      i*vert_increase+vert_increase/3+1);
      // Глаз 2
      MoveToEx(memdc,(j+1)*hor_increase-hor_increase/3,
      i*vert_increase+vert_increase/3, NULL);
      LineTo(memdc,(j+1)*hor_increase-hor_increase/3+1,
      i*vert_increase+vert_increase/3+1);

      Arc(memdc,j*hor_increase+5, i*vert_increase+5, (j+1)*hor_increase-5,
      (i+1)*vert_increase-5, j*hor_increase, i*vert_increase+vert_increase*2/3,
      (j+1)*hor_increase, i*vert_increase+vert_increase*2/3);
...
//================================================================================================================
// Щелчок мышью: 0 - К (нажатие с отпусканием), 1 - Ч (только нажатие) (содрано
// из bkb/Fixation.cpp)
//=================================================================================================================
...
void Indicators::Draw(HDC hdc) {
  ...
  if (flag_training_mode && (training_frame_counter <= 40)) {
    ...
    TextOut(hdc,
            ...
            ypos,
            L"Silence!",
            8);  // при переводе здесь быть осторожным, длина строки изменится
#else
    TextOut(hdc,
            xpos,
            ypos,
            L"Тишина в студии!",
            17);  // при переводе здесь быть осторожным, длина строки изменится
#endif
    ...
    BitBlt(hdc,
           ...
           memdc,
           0,
           WorkerThread::indicator_value * vert_increase,
           SRCCOPY);
  ...
  if (!flag_training_mode) {
    ...
    for (i = 4; i < 6; i++) {
      ...
      switch (confirmed) {
        case 0:  // Обычная отрисовка, ничего не происходит !!! Здесь лучше
                 // красный квадратик, как несостоявшееся нажатие
          if (l > 0)  // Найден К или Ч
          {
            // Но в то ли время он появился?
            if (KChFstate::IsKCValid()) {
              KChWaits[i - 4] = 1;  // Ждём подтверждения, что это короткий звук
                                    // и рисуем обычный зелёный
              BitBlt(hdc,
                     xpos6c[i],
                     ypos6c[i],
                     hor_increase,
            ...
            } else  // Ты пришёл не вовремя!
            {
              ...
              BitBlt(hdc,
                     ...
                     vert_increase,
                     memdc,
                     (num_steps - 1) * hor_increase,
                     num_steps * vert_increase,
                     SRCCOPY);
            }
        ...
        case 1:  // Дождались, можно нажимать. Но только, если нужная буква была
                 // ранее произнесена
          //[28-DEC] если конфликт К и Ч, Ч побеждает
          if ((1 == KChWaits[0]) && (1 == KChWaits[1])) KChWaits[0] = 0;
        ...
        case 2:  // Облом. Рисуем красный прямоугольник. Но только, если нужная
                 // буква была ранее произнесена
          if (1 == KChWaits[i - 4]) {
            // Важно сбросить признак того, что нас ждали
            KChWaits[i - 4] = 0;
    ...
    if ((move[0] != 0) || (move[1] != 0) || (move[2] != 0) || (move[3] != 0)) {
      if (flag_move_mouse)
        MoveMouse(move[2] - move[0],
                  move[1] - move[3]);  // Порядок кнопок - против часовой
                                       // стрелки, начиная с "лево"
    }
  } else  // При включении training_mode надо сделать InvalidateRect диалогу,
          // иначе останутся рудименты
  {
    if (training_frame_counter > 40)  // Выждали паузу, теперь можно записывать
    {
      ...
      int ts = training_sound;
      if (ts < 0 || ts >= MM_NUM_TRAINING_BUTTONS) ts = 0;

      // Для звуков "к" и "Ч" тренировка короче (сейчас одинакова для обоих, но
      // может быть разная)
      ...
      MoveToEx(
          hdc,
          xpos + hor_increase * (WorkerThread::training_silence_indicator + 1),
          ypos,
          NULL);
      LineTo(
          hdc,
