...
volatile int KChFstate::next_kc_counter =
    0;  // Через сколько фреймов можно нажать  К/Ч (в упрощенном режиме)

volatile WORD KChFstate::key_to_press[6] = {
    0xffff, 0xffff, 0xffff, 0xffff, 0xffff, 0xffff};
volatile char KChFstate::repeat_key[6] = {0, 0, 0, 0, 0, 0};
// volatile char KChFstate::toggle_key[6]={0,0,0,0,0,0}; // является ли
// переключателем
volatile char KChFstate::toggle_key[6] = {
    0, 0, 0, 0, 0, 0};  // является ли переключателем

int KChFstate::key_state[6] = {0, 0, 0, 0, 0, 0};  // нажата-отжата
int KChFstate::key_cycle_counter[6] = {
    0, 0, 0, 0, 0, 0};  // Сколько ещё циклов нельзя менять состояние клавиши
...
//==================================================================================================
// // может ли к или Ч быть рассмотрена кандидатом на нажатие (в нужное ли время
// ?) mod. [28-DEC]
//==================================================================================================
bool KChFstate::IsKCValid() {
  if (flag_kc_anytime)  // упрощённый режим
  {
    ...
    // Есть мысль, что и в состоянии 3 всегда valid. Лишь бы уровень сигнала был
    // подходящим. Меняем: return ((2==state)||((3==state)&&(counter<=3));
...
void KChFstate::NewFrame(int energy_level) {
  ...
  if (flag_kc_anytime) {
    ...
    // Декремент вызывается из потока WorkingThread, а проверка на 0 и
    // выставление в 20 - в основном потоке Но атомарности ни там ни здесь не
    // нужно, так как: пропуск одного декремента не критичен, запоздание на один
    // фрейм при нажатии некритично.
  ...
  switch (state) {
    case 0:
      ...
      if (energy_level
          > minlevel + 1)  // снова не тот уровень мы считали базовым для
                           // спокойствия! но уже в сторону повышения.
      {
    ...
    case 1:  // Здесь нам позволено получить всплеск энергии, но не позволено
             // получить понижение энергии
      if (energy_level < minlevel)  // не тот уровень мы считали нижним!
      {
        state = 0;
        KCOnTick(state);  // отладка
      ...
      if (energy_level > minlevel + 1)  // мы выждали паузу и можем перейти в
                                        // состояние 2 при всплеске
      {
#ifdef _DEBUG
        OutputDebugString(L"\r\n***NACHALO!***\r\n");
#endif
      ...
      // Если же ничего из вышеперечисленного не происходит, можем оставаться в
      // этом состоянии хоть вечность. И счетчик нам не нужен.
      KCOnTick(state);  // отладка
      return;
    ...
    case 2:
      if (energy_level
          <= minlevel
                 + 1)  // Это то, чего я ждал! Короткий звук завершился вовремя!
                       // Теперь ждём, что после него тоже будет пауза
      {
        state = 3;
        KCOnTick(state);  // отладка
        counter = 0;
      ...
      counter++;
      // А вот находиться сдесь более 9 фреймов нельзя (восьмой уже пришёл ещё в
      // состоянии 2)... Звук то короткий. Кто долго здесь находится, тот отсюда
      // слетает...
      if (counter > 8) {
      ...
      KCOnTick(state);  // отладка
      return;  // Единыжды это может быть выполнено. Второй раз - уже перебор
               // фреймов короткого звука.

    case 3:
      if ((energy_level > minlevel + 1)
          && counter > 3)  // Не выдержал паузы! Вслед за коротким звуком
                           // вплотную последовал другой. Валим отсюда (3 первых
                           // фрейма прощается).
      {
        // !!! Здесь сбросить ожидающих "к" и "ч"
        Indicators::KChConfirmed = 2;
      ...
      counter++;
      if (counter > 12) {
        // О, сладкий момент! Короткий звук пойман за хвост! Отсюда с почётом
        // уходим в состояние 0
...
void KChFstate::MM_Key(int i, Direction dir) {
  ...
  SendInput(cnt, input, sizeof(INPUT) * cnt);
}
...
LONG KChFstate::TryToPress(int i, LONG move) {
  ...
  // (1/5 секунды)
  if (key_cycle_counter[i] > 0)
    key_cycle_counter[i]--;
  else
    key_cycle_counter[i] = 0;  // Защита от случайного ухода в минус.

  // 0 - клавиша не нажата
  if (0 == key_state[i])  //Нажимаем, если move больше граничного значения.
                          //Пусть это будет 3
  {
    if (move > 3) {
      if (GetToggleKey(i)
  ...
  } else  // Осталось только состояние 2 - временно отжатая клавиша в режиме
          // REPEAT + TOGGLE
  {
