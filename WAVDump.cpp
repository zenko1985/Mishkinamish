...
static FILE *fout =
    0;  // не сделал членом класса, чтобы хедер на зависел от <stdio.h>
...
static GZ_WAVHeader2 wh = {{'R', 'I', 'F', 'F'},
                           0,
                           {'W', 'A', 'V', 'E', 'f', 'm', 't', ' '},
                           16,
                           1,
...
void MMWAVDump::Start(HWND _hdwnd) {
  ...
  GetLocalTime(&st);
  sprintf_s(dump_filename,
            sizeof(dump_filename),
            "MMDump-%d-%d-%d=%d-%02d-%02d.WA_",
            st.wYear,
            st.wMonth,
            st.wDay,
            st.wHour,
            st.wMinute,
            st.wSecond);
  sprintf_s(dump_filename_rename,
            sizeof(dump_filename_rename),
            "MMDump-%d-%d-%d=%d-%02d-%02d.WAV",
            st.wYear,
            st.wMonth,
            st.wDay,
            st.wHour,
