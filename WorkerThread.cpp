...
int volatile WorkerThread::indicator_value;  // принимает значение от 0 до 5 в
                                             // зависимости от уровния сигнала,
...
void WorkerThread::PushData(short *_buf) {
  ...
  } else  //увеличить счётчик пропущенных блоков
  {
    ...
    // Если поток WorkerThread инициализировался уже после того, как  было
    // сгенерировано первое событие SetEvent(hInput2WorkerDataReady), то он
    // никогда не выходил из ступора!!! Вставил сюда
    // SetEvent(hInput2WorkerDataReady), хотя достаточно поменять порядок
    // запуска потоков
    SetEvent(hInput2WorkerDataReady);
  }
}
...
int WorkerThread::PullData(short *_buf) {
  if (MM_WORKER_BUFFERS_IN_CIRCLE
      > num_free_buf)  // какие-то данные в буфере есть, можно забирать
  {
