...
#define MM_WORKER_BUFFERS_IN_CIRCLE 8
...
static volatile LONG num_free_buf =
    MM_WORKER_BUFFERS_IN_CIRCLE;  // количество свободных буферов в кольце

int WorkerThread::last_read_buffer = 0;
int WorkerThread::last_write_buffer = 0;

int volatile WorkerThread::indicator_value;  // принимает значение от 0 до 5 в
                                             // зависимости от уровня сигнала,
                                             // выводится индикатором

uintptr_t WorkerThread::worker_thread_handle = NULL;

short WorkerThread::input_buf[MM_SOUND_BUFFER_LEN];
volatile char WorkerThread::flag_input_buffer_ready = true;
CRITICAL_SECTION WorkerThread::cs_input_buf;
...
void WorkerThread::Start(HWND hdwnd) {
  // Инициализация критической секции для входного буфера
  InitializeCriticalSection(&cs_input_buf);
...
void WorkerThread::Halt(HWND hdwnd) {
  ...
  // 2. Для очистки совести дождёмся окончания потока
  if (worker_thread_handle) {
    WaitForSingleObject((HANDLE)worker_thread_handle, 2000);
  ...
  // 4. Уничтожаем критическую секцию
  DeleteCriticalSection(&cs_input_buf);
}
...
void WorkerThread::PushData(short *_buf) {
  ...
  EnterCriticalSection(&cs_input_buf);
  if (flag_input_buffer_ready)  // место свободно
  {
    ...
    LeaveCriticalSection(&cs_input_buf);

    // 3. Открываем шлагбаум потоку
    if (hInput2WorkerDataReady) SetEvent(hInput2WorkerDataReady);
  } else {
    LeaveCriticalSection(&cs_input_buf);
    //увеличить счётчик пропущенных блоков
#ifdef _DEBUG
...
int WorkerThread::PullData(short *_buf) {
  ...
  if (MM_WORKER_BUFFERS_IN_CIRCLE
      > current_free)  // какие-то данные в буфере есть, можно забирать
  {
    // А свой личный указатель на записанную ячейку - не атомарно, ибо только мы
    // его изменяем
    last_read_buffer++;
    ...
    // Атомарно увеличиваем num_free_buf (гарантированно не превышая
    // MM_WORKER_BUFFERS_IN_CIRCLE, ибо другой поток может только уменьшать
    // num_free_buf)
...
void WorkerThread::Work() {
  ...
  // (здесь не нужно атомарное считывание, ибо только мы сами [Эта ф-ция
  // вызывается из WorkerThread] можем уменьшать количество свободных буферов,
  // а другой поток [ф-ция PullData вызывается из OutputThread] может лишь
  // увеличивать это число)

  // 21.12.2016 Когда выходной поток остановлен, обработка звука прекращалась.
  // Поэтому теперь в любом случае пишем в буфер, просто перезаписывая последнюю
  // область output_buf[last_write_buffer], хотя это и некрасиво (при ВРЕМЕННЫХ
  // тормозах вывода может исказить звук?)
  bool skipped = true;  // пишем в старое место, если буфер забит
  LONG current_free = InterlockedCompareExchange(&num_free_buf, 0, 0);
  if (current_free > 0)  // ой, нет, не забит!
  {
    skipped = false;
    // А свой личный указатель на записанную ячейку - не атомарно, ибо только мы
    // его изменяем
  ...
  if (f_reading_file)
    MMWAVLoader::FillBuffer(
        input_buf);  // При работе с файлом вручную заполняем input_buf

  // 3. Находим максимальный уровень сигнала для индикатора
  // long maxvalue=0;
  // Теперь считаем логарифм среднего квадрата (не спрашивай, почему. где-то
  // что-то слышал, вроде работает).
  ...
  if (average_value <= 0.0) average_value = 1.0;
  ...
  // 3.1 При трейнинге нужно сбросить в 0 training_silence_indicator в начале
  // тренировки 17-DEC
  if (flag_training_mode) {
    ...
    // А потом вникаем в то, что есть уровень тишины. (3/4 секунды слушаем
    // тишину) Были глюки (всплески). Пришлось считать среднее
    ...
    if (40
        == training_frame_counter)  // Пора подбить бабки в вычислении среднего!
  ...
  // 4.1. [27-DEC] Новая концепция - устройство воспроизведения не отключается
  // (из-за необходимости звуков кликов)
  ...
  if (!skipped)  // Начиная с 21.12.2016 буфер может быть перезаписан, и тогда
                 // num_free_buf не уменьшается
  {
    // 5. Перенесено из 1. Ибо OutputThread мог захотеть считать данные ещё до
    // того, как мы реально их записали в output_buf !!!! Корректируем счётчики.
    // num_free_buf - атомарно
  ...
  // При чтении из файла и наличии свободного места - сами себе разрешаем
  // запуститься ещё раз
  current_free = InterlockedCompareExchange(&num_free_buf, 0, 0);
  if (f_reading_file && (current_free > 0)) SetEvent(hInput2WorkerDataReady);
}
